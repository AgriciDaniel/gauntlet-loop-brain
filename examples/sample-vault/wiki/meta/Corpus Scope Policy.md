...
Corpus scope rule: each note states source, retrieval date, harness or domain, supported claims, and explicit non-scope. Each corpus note must state what the corpus proves, what it does not prove, source date, retrieval path, and non-scope boundaries.
