...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Audit Gauntlet Loop Brain maturity gates.")
...
def audit_repo() -> dict[str, Any]:
    ...
    add_category(categories, "market_validation", *check_market_validation())
    ...
    return {
        ...
        "publication_readiness_scope": "not assessed by the repository maturity audit; use the release manifest",
...
def check_market_validation() -> tuple[bool, int, list[str]]:
    path = REPO / "references" / "market-research.md"
    text = read(path).lower()
    if not text:
        return False, 0, ["missing references/market-research.md"]
    pending_markers = [
        "willingness to pay and retention are not yet validated",
        "not yet primary evidence that buyers will pay",
        "market-ready claims remain unavailable",
    ]
    found = [marker for marker in pending_markers if marker in text]
    if found:
        return False, 70, [
            "buyer demand, retention, and equal-budget advantage remain unvalidated; keep market-ready packaging blocked"
        ]
    return True, 100, []
...
def check_adapters() -> tuple[bool, int, list[str], list[str]]:
    ...
    behavior_errors = check_adapter_behavior()
    notes.extend(behavior_errors)
    critical.extend(behavior_errors)
    return not critical, max(0, 100 - 18 * len(critical) - 8 * len(notes)), notes, critical


def check_adapter_behavior() -> list[str]:
    repo_path = str(REPO)
    if repo_path not in sys.path:
        sys.path.insert(0, repo_path)
    try:
        from gauntlet_loop_brain.contract import validate_job
    except ImportError as exc:
        return [f"domain adapter cannot be imported: {exc}"]
    fixture, errors = load_json_object(REPO / "tests" / "fixtures" / "gauntlet-job.json")
    if errors:
        return [f"domain adapter fixture is invalid JSON: {error}" for error in errors]
    failures: list[str] = []
    if validate_job(fixture)[0]:
        failures.append("domain adapter rejects its declared valid fixture")

    missing_goal = json.loads(json.dumps(fixture))
    missing_goal.pop("goal", None)
    if not any("goal" in error for error in validate_job(missing_goal)[0]):
        failures.append("domain adapter does not reject a missing goal")

    for ceiling in ["max_minutes", "max_tokens", "max_cost_usd", "max_retries"]:
        unbounded = json.loads(json.dumps(fixture))
        unbounded["budget"].pop(ceiling, None)
        if not any(ceiling in error for error in validate_job(unbounded)[0]):
            failures.append(f"domain adapter does not reject a job without {ceiling}")

    mutable_gate = json.loads(json.dumps(fixture))
    mutable_gate["gates"][0]["protected"] = False
    if not any("protected" in error for error in validate_job(mutable_gate)[0]):
        failures.append("domain adapter does not reject a mutable protected gate")
    return failures
...
def check_packaging() -> tuple[bool, int, list[str]]:
    ...
    text = path.read_text(encoding="utf-8", errors="replace")
    notes = [f"package script missing {term}" for term in ["SHA256SUMS", "RELEASE_MANIFEST", "release-type", "experimental", "market-ready", "audit_brain.py", "PUBLIC_EXCLUDED", "publication_blockers", "configured_security_url"] if term not in text]
    license_text = read(REPO / "LICENSE").lower()
    if "replace this file with the final license" in license_text:
        notes.append("owner has not selected the final public license")
...
def determine_status(categories: dict[str, dict[str, Any]], score: int, critical: list[str]) -> str:
    ...
    if not categories["market_validation"]["ok"]:
