...
def instantiate_runtime_notes(src: Path, dest: Path, replacements: dict[str, str]) -> None:
    for template_name, output_name in [("hot.template.md", "hot.md"), ("log.template.md", "log.md")]:
        template = src / "wiki" / template_name
        if not template.is_file():
            raise SystemExit(f"ERROR: runtime note template missing: {template}")
        text = template.read_text(encoding="utf-8")
        for old, new in replacements.items():
            text = text.replace(old, new)
        target = dest / "wiki" / output_name
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_text(text, encoding="utf-8")
