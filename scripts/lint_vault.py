...
def main(argv: list[str] | None = None) -> int:
    ...
    errors: list[str] = []
    warnings: list[str] = []
    required = ["CODEX.md", "README.md", "shipping-rules.md", "wiki/index.md", "wiki/overview.md", "wiki/meta/Start Here.md", "wiki/meta/dashboard.md"]
    required.extend(["wiki/hot.template.md", "wiki/log.template.md"] if args.template else [".raw/.manifest.json", "wiki/hot.md", "wiki/log.md"])
    for rel in required:
    ...
    for path in notes:
        ...
        for raw in re.findall(r"!?\[\[([^\]]+)\]\]", text):
            ...
            if key in stems:
                ...
                incoming[rels[key]] += 1
            elif args.template and key in {"hot", "log"}:
                continue
